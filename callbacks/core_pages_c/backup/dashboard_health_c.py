...
def clean_table_callback(url_params, nClicks, pagination, train_no, carriage_no, component_type):
    log.debug(f"[clean_table_callback] 触发源: {callback_context.triggered_id if callback_context.triggered else '初始加载'}")
    ctx = callback_context
    trigger_id = ctx.triggered[0]["prop_id"].split(".")[0] if ctx.triggered else None
    query_train_no = ''
    query_carriage_no = ''
