...
class ChartLineFaultType(Model):
    """chart_line_fault_type视图的ORM模型
    该视图通过UNION ALL组合了dev_view_fault_timed_mat和pro_view_fault_timed_mat两个表的故障类型数据
    """
    dvc_train_no = CharField(max_length=50, verbose_name='车号')
    故障类型 = CharField(max_length=100, verbose_name='故障类型')
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 测试查询最近10条记录
        query = ChartLineFaultType.select().order_by(
            ChartLineFaultType.故障类型, ChartLineFaultType.dvc_train_no
        ).limit(10)
        
        logger.info(f"查询到 {query.count()} 条记录")
        logger.info("字段名称: " + ", ".join(ChartLineFaultType.get_all_verbose_names().values()))

        for i, record in enumerate(query, 1):
            logger.info(f"\n记录 {i}:")
            logger.info(f"车号: {record.dvc_train_no}")
            logger.info(f"故障类型: {record.故障类型}")
