...
class Chart_view_param(Model):
    time_minute = DateTimeField(verbose_name='时间')
    ...
    measurement_count = IntegerField(verbose_name='测量次数')
    avg_value = FloatField(verbose_name='平均值')
    max_value = FloatField(verbose_name='最大值')
    min_value = FloatField(verbose_name='最小值')

    def formatted_time_minute(self):
        """返回上海时区格式化的时间字符串"""
        if not self.time_minute:
            return
        shanghai_tz = timezone('Asia/Shanghai')
        return self.time_minute.astimezone(shanghai_tz).strftime('%Y-%m-%d %H:%M:%S')

    class Meta:
        database = db
        table_name = 'chart_view_param'
        primary_key = False
        schema = 'public'
        indexes = (
            (('dvc_train_no', 'time_minute'), False),
            (('param_name',), False),
        )
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 测试查询最近10条记录
        query = Chart_view_param.select().order_by(Chart_view_param.time_minute.desc()).limit(10)
        ...
        for i, record in enumerate(query, 1):
            ...
            logger.info(f"测量次数: {record.measurement_count}")
            logger.info(f"平均值: {record.avg_value:.2f}")
            logger.info(f"最大值: {record.max_value:.2f}")
            logger.info(f"最小值: {record.min_value:.2f}")
